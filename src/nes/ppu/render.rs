use super::{frame::Frame, background::Background};

pub trait RenderContext<'a> {
  fn get_background(&'a self) -> &'a Background;
}

pub fn rendering_frame<'a, T>(ctx: &'a T) -> Frame where T: RenderContext<'a> {
...
#[cfg(test)]
mod render_tests {
    use crate::nes::ppu::{sprite::build_sprite, tile_position::TilePosition, palette::{PaletteGroup, NES_COLORS}, tile::Tile, background::{Background}};

    use super::{RenderContext, rendering_frame};
  struct TestRenderContext { data: Background }
  ...
  fn rendering_frame_test() {
    ...
    let tile = Tile { sprite, position, palettes };
